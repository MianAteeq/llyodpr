...
class TeamController extends Controller
{
    ...
    {
        $response = Team::orderBy('sort_no','asc')->get();
        return view('team.sort', ['team' => $response]);
    }
    
      public function updateSortTeam (Request $request)
    {
    ...
    public function upload_cv_post(Request $request)
    {
        ...
        if (!empty($request->vcf)) {
            // $vcf = time() . '.' . $request->vcf->extension();
            $vcf = time() . '.vcf';
            $request->vcf->move(public_path('/images'), $vcf);
        } else {
            $vcf = $request->old_vcf;
        }
        $data = [
